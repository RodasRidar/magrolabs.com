...
export class SuscripcionComponent implements OnInit {
  ...
  cancelSubscription(): void {
    ...
    if (confirm('¿Estás seguro de que deseas cancelar tu suscripción? Esta acción no se puede deshacer.')) {
      this.isLoading.set(true);
      this.subscriptionService.cancelSubscription(this.subscription()!.id)
        .subscribe({
          next: (response) => {
            this.subscription.set(response.data.subscription);
            this.isLoading.set(false);
