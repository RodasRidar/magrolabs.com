...
export class ReviewsListComponent implements OnInit {
  ...
  private staticReviews: Review[] = [
    {
      ...
      stars: 5,
      review:
        "Me llego antes de lo esperado, la creatina se ve de calidad y es fácil de llevar al gimnasio.",
    ...
    },
    {
      ...
      stars: 4,
      review:
        "Es buena. La comence a tomar con los desayunos y me siento con más energía y puedo entrenar más duro.",
    ...
    },
    {
      ...
      stars: 4,
      review:
        "Una de las mejores creatinas que he probado, no me ha dado ningún efecto secundario y disuelve muy bien.",
  ...
  private loadReviews() {
    ...
    this._reviewService.getReviewsByProduct(this.productId, true)
      ...
      next: (response) => {
        ...
        this.allReviews = [...dynamicReviews, ...this.staticReviews].sort(
          (a, b) =>
      ...
      error: (error) => {
        ...
        this.allReviews = this.staticReviews.sort(
          (a, b) =>
            new Date(b.created_at).getTime() - new Date(a.created_at).getTime(),
        );
  ...
  private calculateAndEmitStats() {
    ...
    const percentages = {
      ...
      4:
        totalReviews > 0
          ? Math.round((starDistribution[4] / totalReviews) * 100)
          : 0,
      3:
        totalReviews > 0
          ? Math.round((starDistribution[3] / totalReviews) * 100)
          : 0,
      2:
        totalReviews > 0
          ? Math.round((starDistribution[2] / totalReviews) * 100)
          : 0,
      1:
        totalReviews > 0
          ? Math.round((starDistribution[1] / totalReviews) * 100)
          : 0,
    };

    // Calcular promedio con más precisión
    const totalStars = this.allReviews.reduce(
